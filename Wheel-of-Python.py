...
class WOFComputerPlayer(WOFPlayer):
    ...
    def getMove(self,category, obscuredPhrase, guessed):
        letters_to_guess = self.getPossibleLetters(guessed)
        if not letters_to_guess:
            return 'pass'
        elif self.difficulty == 1 and obscuredPhrase.count('_') < 3:
            return phrase

        elif self.smartCoinFlip():
            return sorted(letters_to_guess, key = lambda x: self.SORTED_FREQUENCIES.index(x))[-1]

        else:
            return random.choice(letters_to_guess)
...
while True:
    ...
    print('')
    print('{}(${}) spins...'.format(player.name,player.prizeMoney))
...
if winner:
    ...
    if len(winner.prizes) > 0:
        ...
        for prize in winner.prizes:
            print('    - {}'.format(prize))
    if input("Another Game?(Y/N)").upper() == "Y" or "YES":
        print("Go to work!")
    else:
        print("Bye!")
